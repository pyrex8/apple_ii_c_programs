...
static uint8_t sound_pulses;
static uint8_t sound_delay;
...
void score_set(uint8_t value)
{
    score_ones = digit_ones_get(value);
    score_tens = digit_tens_get(value);
    score_hundreds = digit_hundreds_get(value);
    score_draw();
}

void high_score_set(uint8_t value)
{
    high_ones = digit_ones_get(value);
    high_tens = digit_tens_get(value);
    high_hundreds = digit_hundreds_get(value);
    high_score_draw();
}
...
void main(void)
{
    ...
    while(1)
    {
        ...
        if (paddle_update == PADDLE_UPDATE_DELAY)
        {
            ...
            score_set(sound_pulses);
            high_score_set(sound_delay);
        }
